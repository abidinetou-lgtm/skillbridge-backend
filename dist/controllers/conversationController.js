...
const createConversationController = async (req, res, next) => {
    try {
        ...
        const data = await (0, conversationService_1.createConversationService)(userId);
...
const readConversationController = async (req, res, next) => {
    try {
        ...
        const userId = getAuthenticatedUserId(req);
        const data = await (0, conversationService_1.readConversationService)(conversationId, userId);
        res.status(200).json(data);
    }
    catch (error) {
        next(error);
